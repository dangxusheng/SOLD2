"""
This file implements the yorkurban dataset object for pytorch.
"""
import os
import math
import copy
...
def yorkUrban_collate_fn(batch):
    """ Customized collate_fn for yorkUrban dataset. """
...
class YorkUrbanDataset_NoGT(Dataset):
    def __init__(self, mode="train", config=None):
        super(YorkUrbanDataset_NoGT, self).__init__()
        if not mode in ["train", "test"]:
            raise ValueError(
                "[Error] Unknown mode for yorkUrban dataset. Only 'train' and 'test'.")
        self.mode = mode
        ...
        print("[Info] Initializing yorkUrban dataset...")
    ...
    def get_filename_dataset(self):
        ...
        dataset_path = []
        if self.mode == "train":
            dataset_path = [cfg.yorkUrban_dataroot]
        elif self.mode == "test":
            dataset_path = [cfg.yorkUrban_dataroot]
        ...
        for folder in dataset_path:
            import glob
            image_paths.extend(glob.glob(f'{folder}/*/*.jpg',recursive=True))
        image_paths = sorted(image_paths)

        assert len(image_paths) > 0, f'image_paths size is 0...'
