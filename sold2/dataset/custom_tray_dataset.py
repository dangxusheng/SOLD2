"""
File to process and load the tray dataset.
"""
import os
import math
import copy
...
class TrayDataset(Dataset):
    def __init__(self, mode="train", config=None):
        ...
        if not mode in ["train", "test"]:
            raise ValueError(
        "[Error] Unknown mode for Tray dataset. Only 'train' and 'test'.")
        self.mode = mode
        ...
        self.cache_path = cfg.tray_cache_path
        ...
        if "gt_source_%s"%(self.mode) in self.config:
            ...
            print(f'gt_source: {self.gt_source}')
        ...
        print("[Info] Initializing Tray dataset...")
    ...
    def get_filename_dataset(self):
        ...
        if self.mode == "train":
            # Contains xxx images
            dataset_path = [os.path.join(cfg.tray_dataroot, p)
                            for p in self.config["train_splits"]]
        else:
            # Test mode
            dataset_path = [os.path.join(cfg.tray_dataroot, p)
                            for p in self.config["test_splits"]]
